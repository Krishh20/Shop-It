...
export default function Home() {
  ...
  const [selectedCategory, setselectedCategory] = useState("All");  // add filters based on price range,ratingf
  const { cart } = useCart();
  console.log({ cart });
  useEffect(() => {
    (async () => {
  ...
  const filteredCategories=getProductsbyCategory(products,selectedCategory) //go through
  return (
    <>
      <Navbar />
      <main className="!pt-8">
        <div className="flex gap-4 justify-center !mb-4">
          {categories.map((category) => (
            <div
              key={category.id} // or `category.name` if `id` not available
